            ...
            <div class="card mb-4 shadow-sm">
                <div class="row g-0">
                    <div class="col-md-2 d-flex align-items-center justify-content-center">
                        <img src="{{ asset('gambar/' . $orderItem->produk->gambar) }}" alt="Gambar Produk" class="img-fluid rounded" style="max-width: 80%;">
                    </div>
                    <div class="col-md-10">
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Harga:</strong> Rp {{ number_format($orderItem->harga, 0, ',', '.') }}</p>

                            <form action="{{ route('order.review.store', $order->id) }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                    <label for="rating-{{ $orderItem->id }}" class="form-label">Rating (1-5)</label>
                                    <select name="rating" id="rating-{{ $orderItem->id }}" class="form-select" required>
